...
def test_eleven_real_customizations_form_closed_registry():
    reg, catalog, manifests = _load_all()
    assert reg.active_ids() == tuple(
        f"BANK-OM-{number:03d}" for number in range(1, 12)
    )
...
def test_real_snapshot_readiness_is_honestly_blocked():
    ...
    assert sum("owner is not assigned" in reason for reason in result.reasons) == 11
