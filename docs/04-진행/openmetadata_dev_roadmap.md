...
> 비개발자의 결과 해석과 운영 시나리오는
> [`비개발자용 사용 가이드`](../00-사용가이드/비개발자_사용_가이드.md)에 별도
> 정리하며, 사용자에게 보이는 상태·입력·절차가 바뀔 때마다 함께 갱신한다.
...
- **T61 patch-kill** — high/critical 우선·상태 4종(killed/survived/inconclusive/
  ...
  테스트 방지). 현재 Sybase/Tibero source negative control 2/5 pass, API 기반
  high ID 3개는 제거본 runtime 배포 대기.
...
| T61 patch-kill | 🟡 source 2/5 + runtime 1/5 증거(계 3/5) | `acgh/patchkill.py` + source/runtime plan·runner·workflow | Sybase/Tibero 고정 predecessor에서 실제 failure 입증(source 2/5). **2026-08-10 BANK-OM-001 runtime negative control 실행 완료** — 계획서 지정 전임 `afcb2d2c`(공식 1.13.1) 배포본에서 `test_instance_code` 2회 연속 fail(404), health probe 3종 정상, 증거 `evidence/om-1.13.1-runtime-t61-20260810/`. BANK-OM-002·003은 전임 이미지(`a2566fac`·`4108411c`) 미빌드로 미완. **BANK-OM-005는 필수 test가 순정에서도 pass해 생존 미입증(신규 결함)** |
...
> 7-ID snapshot 재구성과 4개 candidate-follow-up을 합친 현재 11-ID candidate,
...
> `21bfc15`에서 정정.) **다음 = MVP2 운영 증거 생성**: 미배정 owner와 승인자를
> 지정하고, 배포된 후보·제거본에서 T61/T62를 실행한 뒤 실제 T90 업그레이드,
> T91 동일 digest 승격, T94 내부망 서명 재검증을 수행한다. T43·T51/52·T80은
> 기본 엔진이 구현됐으며 운영 데이터 기반 임계치·범용 provider·실제 memo 평가
> 데이터 보강이 남았다. 검증기 카탈로그 §0.1 구현현황 참조.
