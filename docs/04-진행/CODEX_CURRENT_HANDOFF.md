...
## 25. 2026-08-09 사람·LLM 동등 검사 최종 목적 재정의

유사도 후보 설계에 대한 Claude 후속 검토는 안 D(후보 검색 제거)에 반대하고
안 B(검색 유지·판정 영향 제거)를 권고했다. 실제 1.13.2에서는 전체 파일에서 후보를
줄여 정답 조사에 도움을 줬지만, 현재 구현은 source 후보 하나가 기존 정의 실패를
BLOCK에서 APPROVAL로 완화한다. 기능 test 증거도 Candidate SHA·role에는 묶이지만
BANK-OM ID·definition·새 경로·assertion에는 충분히 결속되지 않는다. BANK 기능이
없는 symbol 유사 후보를 APPROVAL로 기대하는 synthetic test도 존재한다. Codex가
해당 코드와 fixture를 다시 대조해 세 P0의 사실관계를 확인했으며 아직 수정하지
않았다.

사용자는 최종 목적을 다음처럼 확정했다.

1. LLM은 업그레이드·충돌 해소·코드 이관을 수행할 수 있는 선택적 작업자다.
2. 검사기는 LLM의 판단을 믿지 않고 승인된 관리파일·Git·assertion·필수 test로
   PASS·BLOCK 계열 판정을 독립 생성한다.
3. LLM이 없어도 사람이 같은 관리파일과 명령으로 전체 절차를 실행·해석·복구할 수
   있어야 한다.
4. 같은 정책·Candidate·검사기 버전이면 실행 주체가 사람인지 LLM인지와 무관하게
   canonical verdict와 digest가 같아야 한다.
5. LLM은 제품 코드와 test를 수정할 수 있지만, 같은 실행에서 정책을 완화해 자기
   결과를 승인할 수 없다.

이 목표와 현재 구현의 신뢰 경계·복잡성을 Claude에게 다시 적대적으로 검토하도록
다음 요청서를 작성했다.

```text
docs/04-진행/구조보완_최종목표_사람LLM동등검사_CLAUDE_설계검토요청_20260809.md
```

요청서는 Manifest·Contract·shared-code definition·test·Candidate lock·Phase
digest의 역할, 사람·LLM 동등성, 정책 자기승인, test 약화, 기능 증거 재사용,
BLOCK 완화, 사람 단독 운영성을 30개 질문과 16개 필수 반례로 검토한다. 모든 기능을
`판정 필수 / 조사 보조 / 사람 전용 / 제거 후보`로 분류하고 최소 관리파일·gate·
상태 전이를 제안하도록 요구한다. Claude 결과와 Codex 재대조 전에는 안 B나 새로운
관리 schema를 구현하지 않는다. 예행연습 Candidate·기존 Phase 증거도 수정하지
않았다.
