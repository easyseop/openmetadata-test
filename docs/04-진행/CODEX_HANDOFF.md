...
7. 2차 검사기 원리 페이지의 녹색 체크를 복구했다. 체크는 “검사기 코드와
   관련 단위 테스트 구현 확인”만 뜻하며 특정 후보 PASS나 행내 환경 실행
   완료를 뜻하지 않는다는 범례를 추가했다. 환경 연결·운영 증거·추가 보강은
   체크와 별도의 상태 문구로 표시한다.
...
9. 1.13.1 충돌 화면 앞에 공통 기준 1.13.0, 공식 1.13.1 변경,
   ...
   같은 번역 값을 다르게 고친 것이 아니라 같은 객체의 같은 줄 주변을
   변경해 발생한 줄 단위 충돌이며, 동일 JSON 항목의 동시 변경은 0개였음을
   명시했다.
