...
- 초록 원형 체크와 연한 초록 배경은 “구현 완료 + 현재 실제 결과 확인 가능”을
  뜻한다. PASS 표시가 아니므로 APPROVAL 결과인 T63도 체크 대상이다.

### 초록 체크 분류

- A 8개: T25-R, T25, T26, T30, T31, T40, T41, T93 정확 범위
- B 2개: T42, T93 경로 정책
- C 2개: T60-I, T63
- D 0개

합계 12개다. 현재 결과까지 완성되지 않은 항목은 T43, T61, T62, T90, T91
다섯 개다.

### 남은 구현·외부 입력

`운영 적용 전에 우선 보완할 항목`에 다섯 검사기의 남은 일과 완료 기준을
추가했다.

1. T43: 실제 재적용 증거에서 충돌률 자동 계산 및 증거 digest 결속
2. T61: BANK-OM-001~003 제거본을 행내 환경에 배포해 의도한 실패와 포함본
   통과를 같은 candidate SHA에 결속
3. T62: 행내 URL·인증·fixture·artifact와 환경 digest로 대기 중인 Contract
   test 7개 실행
4. T90: 12단계 실제 업그레이드와 단계별 로그·담당자·SHA 기록
5. T91: 이미지·Helm·검사 결과 digest와 실제 승격 대상을 서명해 비교

T41, T42·T93, T60-I, T63은 결과가 있으나 정적 보안 분석, 간접 의존,
JUnit/Jest/direct_tests, 실행환경·로그 digest 등 운영 범위 보강이 남아
별도 목록으로 분리했다.

### 검증

- 생성된 phase2 preview에 상세보기 17개, 초록 표시 12개, 지도 카드 4개,
  범례 포함 체크 아이콘 13개가 있음을 구조 검사로 확인했다.
