...
> 대응 상태: 0단계 기준선 복구 완료 · 1단계 읽기 전용 분석기 대기

## 결론

검토 결과를 수용했습니다. `plan`·`apply` 자동화 본체를 바로 구현하지 않고,
현재 Manifest v2 등록자료와 소스 검사가 다시 실행 가능한 상태부터
복구합니다.
...
6. symlink·submodule·Git LFS 차단은 현재 미구현이라는 사실을 명시했습니다.
...
- `prepare_registration.py plan`
- 차단 판정기 전체
- proposal·review-required 생성
- digest 승인과 동시 적용 잠금
- Candidate lock의 source-tree/build-artifact 종류 구분
- upstream에 없는 행내 신규 경로를 watch에서 분리하는 정책과 코드
- symlink·submodule·LFS mode 검사
- OM_TEMP 1.13.1 재검증

1.13.1은 원격 `patch/om-1.13.1`·`custom/om-1.13.1` 후보가 없으므로 현재
노트북에서 결과 파일을 갱신하지 않습니다. 기존 1.13.1 T41 결과는 새 코드로
재실행하기 전까지 과거 결과로 취급해야 합니다.

## 다음 구현 순서

1. 읽기 전용 Git 분석기
2. 차단 조건과 edge case test
3. 제안서와 사람 판단 목록
4. 승인·적용 기능

쓰기 기능은 1번과 2번의 판정 기능이 모두 통과하기 전에는 추가하지 않습니다.
