...
- `docs/00-사용가이드/예행연습-1.13.1-1.13.2/OM_TEMP_1.13.1_113개_변경파일_기능분류_가이드.md`

검토 기준은 다음과 같다.

1. 실제 OM_TEMP branch diff가 113개이며 추가 43·수정 70인지 확인한다.
2. 각 경로가 정확히 한 번 나오고 전용 74·공용 37·제외 2인지 확인한다.
3. 공용 37개 경로의 `(파일 경로, BANK-OM ID)` 조합 114개가 모두 고유하며
   BANK-OM-001~007 Manifest와 일치하는지 확인한다.
4. 시연용 다중 연결 승인을 운영 승인으로 과장하지 않았는지 확인한다.
5. 공용 코드 정의 초안의 빈 assertions가 검사 가능·schema 통과 상태로
   표현되지 않았는지 확인한다.

실제 코드 정의와 BANK-OM-001~007 업무 분류는 아직 승인 전이다. Claude는 빈
assertions를 임의의 이름이나 짧은 token으로 채우지 말고 실제 OM_TEMP 1.13.1
diff의 완전한 코드 조각 또는 JSON·YAML pointer·값을 근거로 제안해야 한다.
...
## 13. 2026-07-29 공유 HTML 가독성 개편 검토 요청

### 변경 목적

처음 보는 독자가 긴 검사기 표에서 막히지 않도록 정보의 양은 유지하면서 공개
순서를 바꿨다. 1차는 현재 쓰는 저장소만 보여주고, 2차는 검사기 종류를 먼저
훑은 뒤 필요한 설명만 펼쳐 읽게 한다.

### 핵심 변경

- 1차의 과거 `easyseop/OpenMetadata` 저장소 역할 카드와 세 저장소 흐름을
  제거하고, 현재 OM_TEMP 제품 재현과 governance 검사 저장소의 두 역할만
  남겼다. 과거 실제 diff의 출처 링크는 증거 상세에만 유지했다.
- `candidate_additional_paths`를 실제 BANK-OM-007의 최초 8개 + 후속 2개
  사례로 교체했다. 최초 allowed를 고치면 T25-R의 역사적 재구성 기준이
  변한다는 이유, 현재 T26/T40/T93은 10개 합집합을 본다는 동작,
  `series.allowed`와 required 조건을 함께 설명했다.
- 2차 상단에 4개 목적별 검사기 지도를 추가하고 T번호가 화면 순번이 아니라
  전체 실행계획의 안정적인 태스크 ID임을 설명했다.
- 기존 긴 방법·예외 표의 문장은 삭제하지 않고 17개 독립 상세보기로 옮겼다.
  각 상세는 목적·실제 검사·예외/보완으로 분리했다. T93은 exact scope와
  policy drift 역할이 달라 두 상세로 유지했다.
- 전체 입력·출력 표는 맨 뒤의 별도 상세보기로 이동했다.

### 검토할 쟁점

1. BANK-OM-007의 `allowed_changed_paths` 8개와
   `candidate_additional_paths` 2개 분리가 `manifest.py`, `drift.py`,
   `survival.py`, `policy_drift.py`의 현재 의미와 일치하는가.
2. 검사기 지도에서 T60-I를 테스트 그룹에 보여주면서 실제 상세는 소스 기준
   묶음에 둔 것이 독자에게 혼동을 주는가.
3. T93을 두 상세보기로 나눈 것이 구현의 두 사용 문맥을 정확히 설명하는가.
4. 모바일에서 summary의 두 줄 구조와 펼친 3칸이 1열로 바뀌는 CSS가 충분한가.
5. 과거 저장소 카드 제거 후에도 남은 실제 commit 출처 링크가 현재 작업
   저장소로 오해되지 않는가.
...
- A/B/C/D 지도에는 각 묶음의 현재 사용 범위를 함께 표시한다.
- 초록 원형 체크는 구현 완료만으로 붙이지 않고, 현재 저장된 실제 결과까지
  확인 가능한 경우에만 붙인다.
- 체크는 PASS가 아니다. 현재 판정이 APPROVAL인 T63도 “결과 확인 가능”이므로
  체크했고, 구현 코드가 있어도 실제 행내 결과가 없는 T61/T62/T90/T91과
  수동 충돌률 입력을 쓰는 T43은 체크하지 않았다.

현재 분류는 A 8개(T25-R/T25/T26/T30/T31/T40/T41/T93 exact), B 2개
(T42/T93 policy), C 2개(T60-I/T63), D 0개로 총 12개다. T60-I 상세는
지도와 맞게 A에서 C로 이동했다.

Claude는 다음을 독립적으로 확인해 달라.

1. 12개 체크 항목마다 저장소에 현재 결과 파일 또는 재현 가능한 현행 증거가
   실제로 존재하는가.
2. T43/T61/T62/T90/T91을 체크하지 않은 판단이 구현·운영 증거 경계와
   일치하는가.
3. `운영 적용 전에 우선 보완할 항목`의 남은 일과 완료 기준이 코드의 실제
   미구현 또는 외부 입력 blocker와 일치하는가.
4. 초록색을 PASS로 오해하지 않도록 범례와 실제 3차 판정표의 관계가 충분히
   분명한가.
5. 모바일과 데스크톱에서 체크가 제목의 맨 오른쪽에 보이고, 상세 본문 대비가
   읽기 가능한가.
...
이 가이드는 사용자 요청으로 폐기·삭제했으며 더 이상 Claude 검토
대상이 아니다. 기존 예행연습 문서와 Phase 구현은 이 삭제로 변경되지
않았다.
