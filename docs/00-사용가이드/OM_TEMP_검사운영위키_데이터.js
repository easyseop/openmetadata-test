...
window.WIKI_FILES = {
  manifest: {
    ...
    commands: [
      {
        label: "초안 재생성 · 검토용 폴더의 Manifest가 바뀜",
        meaning: "Git commit 이력에서 BANK-OM ID별 changed_paths 초안을 다시 만듭니다. 예를 들어 BANK-OM-007의 두 commit이 8개와 2개 파일을 변경했다면 한 Manifest의 changed_paths 10개로 합칩니다. 실제 Manifest를 바로 덮어쓰지 않도록 검토용 출력 폴더를 지정합니다. 이미 Manifest를 직접 수정했다면 반드시 실행할 명령이 아닙니다.",
        command: "python harness/registrations/om-temp-1.13.0/generate_manifest_drafts.py \\\n  --repo /path/to/OM_TEMP \\\n  --output /tmp/om-temp-manifest-review"
      },
      {
    ...
    storage: "버전별 등록 폴더에서 Git 이력으로 관리합니다. 새 공식 버전에서도 같은 업무 기능이면 같은 BANK-OM ID를 유지하되 새 버전 폴더의 changed_paths를 다시 생성·검토합니다. 최초 구현과 후속 수정의 구분은 Manifest 목록을 나누지 않고 Git commit SHA와 커밋 순서로 확인합니다. 생성 명령에서 --output을 생략하면 실제 manifests 폴더가 기본 출력이므로 기존 파일을 덮어쓸 수 있습니다."
  },
  registry: {
...
window.WIKI_TOPICS = {
  ...
  registration: {
    ...
    sections: [
      ...
      ["5. 등록 묶음 생성", "Registry, 공용 파일 소유정보와 전체 변경 목록을 다시 생성합니다."],
      ["6. 검사", "등록자료 검사와 소스 검사를 실행하고 BLOCK 원인을 수정합니다."]
    ],
    example: "python harness/registrations/om-temp-1.13.0/generate_manifest_drafts.py --repo /path/to/OM_TEMP\npython harness/registrations/om-temp-1.13.0/generate_registration_bundle.py --repo /path/to/OM_TEMP\npython harness/registrations/om-temp-1.13.0/validate_registration_bundle.py --repo /path/to/OM_TEMP",
    ...
    update: "같은 ID의 후속 commit에서 새 파일이 확인되면 해당 버전 Manifest의 changed_paths를 갱신하고 Registry·공용 소유정보·전체 변경 목록을 재생성합니다.",
  ...
  followup: {
    ...
    sections: [
      ...
      ["6. 파생 파일·검사 갱신", "Registry 연결, 공용 소유정보와 전체 변경 목록을 재생성하고 T30·T31·T40·T42·T93을 다시 실행합니다."]
    ],
    example: "git commit -m \"Tibero 연결 파일 누락 보완\" \\\n  -m \"Customization-ID: BANK-OM-007\"\n\n# 변경 전: 8개\nimplementation:\n  changed_paths:\n    - openmetadata-spec/.../tiberoConnection.json\n\n# 변경 후: 기존 8개 + 새 파일 2개\nimplementation:\n  changed_paths:\n    - openmetadata-spec/.../tiberoConnection.json\n    - openmetadata-ui/.../serviceConnection.ts\n    - openmetadata-ui/.../DatabaseServiceUtils.test.tsx",
    resultExample: "후속 commit 7d19c895…를 별도 기능으로 만들지 않고 BANK-OM-007에 연결\n→ series.allowed=true 확인\n→ changed_paths 8개에서 현재 전체 10개로 갱신\n→ 등록 범위 검사 PASS",
    update: "Manifest, 필요한 경우 Contract·Registry, shared-path-owners.yaml과 source-diff-paths.txt를 갱신합니다. patch-replay 전략에서만 Patch-lock revision과 source_commits도 추가합니다.",
  ...
  automation: {
    ...
    summary: "목표 자동화는 commit을 감지해 경로 초안과 파생 파일을 만들고 검사를 시작하는 것입니다. 현재 생성 스크립트는 있지만 OM_TEMP commit과 검사 저장소 갱신이 완전히 자동 연결되지는 않았습니다.",
    sections: [
      ["자동화할 항목", "변경 경로, BANK-OM ID 형식, Manifest 경로 초안, Registry 연결, 공용 파일 소유정보, 전체 변경 목록과 스키마 검증"],
      ["사람이 결정할 항목", "required 파일, 간접 watch 의존, Contract invariant와 test, owner·criticality, 충돌 해결 코드와 최종 승인"],
      ["후보 확정 뒤 자동 생성", "Candidate lock은 commit과 빌드 artifact가 모두 확정된 뒤 생성합니다."],
      ["환경 test 때 자동 생성", "Test run set과 검사 결과는 실행마다 새로 만들고 기존 결과를 덮어쓰지 않습니다."]
    ],
    example: "OM_TEMP 코드 commit\n  → 자동 diff·Manifest 변경안\n  → 담당자 의미 확인\n  → 등록 묶음 재생성·검사\n  → Pull Request 또는 BLOCK",
    resultExample: "현재: 담당자가 생성 스크립트를 직접 실행하고 결과를 검토\n목표: commit 감지 → 자동 변경안 생성 → 담당자 승인 → 검사 통과 시 병합 가능\n사람 판단: required·Contract·간접 watch·충돌 해결은 자동 확정하지 않음",
