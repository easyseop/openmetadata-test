...
**전체 순서:** 1/11 · [전체 목차](./OM_TEMP_1.13.1_1.13.2_예행연습_전체목차.html)
...
이 가이드를 다른 컴퓨터에서도 그대로 사용할 수 있도록 사용자 이름이 들어간 절대경로를 사용하지 않습니다. 검사기 저장소를 clone한 폴더에서 다음을 실행합니다.
...
`OM_CODE_REPO`는 기본적으로 `$HOME/om-work/om-temp-real-1.13.1`, `KB_SOURCE_REPO`는 `$HOME/om-work/kb_openmetadata`를 가리킵니다. 다른 위치를 사용하면 `source`를 실행하기 전에 해당 환경변수를 지정합니다.
...
| `$OM_CODE_REPO` | 이번 1.13.1 복원 예행연습용 폴더입니다. | 없으면 새로 만들고, 있으면 다시 clone하지 않습니다. |
...
```bash
git -C "$KB_SOURCE_REPO" rev-parse refs/remotes/origin/main
```
...
```bash
git -C "$KB_SOURCE_REPO" rev-parse refs/tags/1.13.1-release
```
...
**수행 내용**
다음 clone 명령이 `OM_CODE_REPO`를 만들 수 있도록 상위 폴더를 준비합니다.

```bash
mkdir -p "$(dirname "$OM_CODE_REPO")"
```

이 명령은 상위 폴더가 없을 때만 생성하고 기존 코드를 변경하지 않습니다.
...
```bash
git clone --filter=blob:none --no-checkout https://github.com/easyseop/OM_TEMP.git "$OM_CODE_REPO"
```

`--filter=blob:none`은 처음부터 모든 대용량 파일을 받지 않고 필요할 때 받습니다. `--no-checkout`은 아직 코드 파일을 펼치지 않는다는 뜻입니다.

**예상 결과**
`$OM_CODE_REPO` 폴더가 생성됩니다. 기존 `$HOME/om-work/OM_TEMP`는 수정되지 않습니다.
...
```bash
git -C "$OM_CODE_REPO" remote add kb-source "$KB_SOURCE_REPO"
```
...
```bash
git -C "$OM_CODE_REPO" fetch kb-source refs/remotes/origin/main:refs/remotes/kb-source/main refs/tags/1.13.1-release:refs/tags/1.13.1-release
```
...
```bash
git -C "$OM_CODE_REPO" config user.name
```

```bash
git -C "$OM_CODE_REPO" config user.email
```

두 명령 중 하나라도 빈 출력이면 작성자 정보를 설정한 뒤 진행합니다. 다른 사람의 이름이나 이메일을 임의로 입력하지 않습니다.

**작성자 정보 미설정 시 조치**
아래 설정은 `$OM_CODE_REPO`에만 적용됩니다. 다른 Git 저장소의 작성자 정보는 바뀌지 않습니다.

먼저 첫 번째 명령의 `JISEOP LEE`를 commit에 표시할 본인 이름으로 확인하거나 바꾼 뒤 실행합니다.

```bash
git -C "$OM_CODE_REPO" config --local user.name "JISEOP LEE"
```
...
```bash
git -C "$OM_CODE_REPO" config --local user.email "YOUR_GITHUB_EMAIL"
```
...
```bash
git -C "$OM_CODE_REPO" config --local --get user.name
```

```bash
git -C "$OM_CODE_REPO" config --local --get user.email
```
...
**문서 이동:** [다음 단계 — 로컬 branch 연결 및 검증](./OM_TEMP_1.13.1_로컬브랜치_연결_및_검증_가이드.html)
