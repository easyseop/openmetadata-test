...
### 충돌한 것은 전부 번역 문구 파일입니다

8건 전체에서 한 번이라도 충돌한 파일을 모아 보면 **18개, 전부 번역
파일**입니다. Java 도 화면 코드도 설정 파일도 하나 없습니다.

```text
ar-sa  de-de  es-es  fr-fr  gl-es  he-he  ja-jp  ko-kr  mr-in
nl-nl  pr-pr  pt-br  pt-pt  ru-ru  th-th  tr-tr  zh-cn  zh-tw
```

| 기능 | 충돌 |
|---|---|
| BANK-OM-001 ~ 004 | 각각 **같은 18개** |
| BANK-OM-005 ~ 007 | **0개** |

### 왜 하필 번역 파일만인가 — 내용 다툼이 아닙니다

언어 파일은 **19개**인데 충돌한 것은 **18개**입니다. `en-us.json` 하나만
빠졌습니다. 그 차이를 따라가면 원인이 나옵니다.

**OpenMetadata 1.13.1 이 번역 파일 18개의 들여쓰기를 2칸에서 4칸으로
바꿨습니다.**

```text
1.13.0 의 ko-kr.json          1.13.1 의 ko-kr.json
{                             {
  "label": {                      "label": {
...
| 파일 | 1.13.0 | 1.13.1 | git 이 본 변경 | 충돌 |
|---|---|---|---|---|
| `ko-kr.json` 외 17개 | 2칸 | **4칸** | **3,430줄 추가 / 3,346줄 삭제** | **남** |
| `en-us.json` | 2칸 | 2칸 | 111줄 추가 / 28줄 삭제 | 안 남 |

**내용이 늘어난 양은 둘 다 비슷합니다** — `ko-kr` 은 항목 84개, `en-us` 는
83개가 늘었습니다. 새 문구가 들어온 양은 같은데, **18개 파일만 형식이
바뀌어서 git 이 파일 전체를 다시 쓴 것으로 본 것**입니다.

> **그래서 이 18개 충돌은 "우리 기능과 새 버전이 다툰 것"이 아닙니다.**
> 새 버전이 파일 모양을 바꿔서 git 이 통째로 겹쳐 본 것뿐이고, 항목 단위로
> 갈라 보면 겹치는 것이 **0개**입니다. 발표에서 "충돌 18개"라는 말이
> 위험해 보인다는 질문이 나오면 이렇게 답하시면 됩니다.

### 이번에 운이 좋았던 것도 사실입니다

**JSON 이 아닌 파일이 충돌하면 자동 해결 대상이 아닙니다.** 그때는 그
자리에서 멈추고 코드 담당자가 직접 해결해야 합니다.
...
이번 1.13.0 → 1.13.1 에서는 Java·화면 코드가 하나도 충돌하지 않아 그 경로가
쓰이지 않았습니다. **다음 버전에서도 그러리라는 보장은 없습니다.**
