...
오른쪽 말로 통일했습니다. **화면 글자와 문서의 말이 다를 수 있습니다.**
...
똑같이 이름표 하나로 관리된다**는 것을 보여주려고 둘을 고릅니다.
...
> <sub>56~58줄이 노랗게 강조된 부분. G6 세 장 중 이 화면이 가장 중요합니다</sub>
>
...
| | 확인하는 것 | 어디서 나오나 |
|---|---|---|
| 1 | 공식이 바꾼 834개가 **우리 감시 목록과 어디서 겹치는가** | 구간 3 |
...
| 시점 | 알 수 있는 것 | 어디서 |
|---|---|---|
| **합치기 전** (지금) | 감시 목록과 **겹치는 파일이 있는가** | **구간 3** |
...
> <sub>화면 맨 아래. `upstream` 세 줄과 그 위의 `gate` 가 나오는 부분</sub>
...
**이 표가 구간 3의 결과물입니다.** OpenMetadata 가 834개를 바꿨지만, 기준코드
...
바뀌었습니다. 감시 목록을 파일 단위로 넓게 잡아 두는 이유가 이것입니다.
...
| 이름 | 뜻 |
|---|---|
| `name` | 어느 검사인지. `upgrade-watch` = "우리 감시 목록의 파일이 이번에 바뀌었나" |
...
> **한 줄로 먼저 답하면** — `cherry-pick` 은 **파일 내용 때문에** 쓰는 것이
> 아닙니다. 머지해도 최종 파일 내용은 똑같이 나옵니다(아래에서 실측으로
> 확인했습니다). **"기록 8건 · 이름표 1개씩"이라는 구조를 유지하려고** 씁니다.
...
머지를 끝까지 진행한 뒤 기록을 확인하면 차이가 드러납니다.
...
**충돌을 푼 결과가 이름표 없는 머지 기록 하나에 전부 들어갑니다.**
...
이유 중 가장 중요한 것이 이것입니다. 뒤에 오는 검사기는 후보를 이렇게 읽습니다.
...
> **여기서 뜻을 정확히 잡아야 합니다.** "1.13.0 이 딸려 온다"는 것은 **파일이 섞여
...
읽어 봐야 합니다. **"한글 입력 기능만 빼 주세요" 라는 흔한 요청에
답하려면 매듭 안을 먼저 들여다봐야 합니다.**
...
이름표가 없습니다. 코드는 정상으로 돌아가도 **그 변경을 설명할 수 없습니다.**
...
`BANK-OM-005`(한글 입력 보정) 전문입니다. 등록표 7장 중 가장 짧아 전체를 한 화면에 실을 수 있습니다.
