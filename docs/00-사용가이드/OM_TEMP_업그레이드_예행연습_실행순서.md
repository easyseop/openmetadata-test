...
> 로 아예 거부되고, 억지로 붙이면 **734개**가 부딪힙니다. 위의 18개는
...
그럼 왜 "부딪혔다"고 나오나 — **git 은 같은 파일에 양쪽이 손댔다는
것까지만 보고 내용은 안 보기 때문입니다.** 파일 단위로는 부딪혔지만 항목
...
| **JSON이 아닌 파일이 부딪힘** | **멈춤** — 자동 처리 대상이 아님 |
...
```
중단: BANK-OM-001: JSON 이외 파일이 부딪혔습니다. 자동 정리 대상이 아니므로
    코드 담당자가 직접 해결해야 합니다
```
...
| 열 | 단위 | 보는 법 |
|---|---|---|
| 부딪힌파일 | 파일 개수 | 몇 개가 부딪혔나 |
| 정리한파일 | 파일 개수 | 그중 몇 개를 처리했나 — **부딪힌파일과 같아야 함** |
| 되살린항목 | **항목 개수** | 단위가 다릅니다. 비교 대상이 아닙니다 |

`18 = 18` 이면 **부딪힌 것을 하나도 남기지 않고 정리했다**는 뜻입니다.
