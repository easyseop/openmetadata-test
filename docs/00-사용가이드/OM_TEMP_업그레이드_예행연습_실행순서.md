...
**따로 돌립니다. 그리고 이미 돌린 결과가 저장소에 들어 있습니다.**
...
부딪힌 18개가 전부 번역 문구 파일이었던 이유도 여기 있습니다. 새 버전이
19개 언어 파일에 문구를 추가했고, 우리도 같은 파일들에 문구를 넣었기
때문입니다. 나머지 30개(`InstanceCodeResource.java` 등)는 새 버전이
건드릴 이유가 없어서 조용히 들어갔습니다.
