...
> 📸 **④**
>
> ![구간 0·1·2 연속 화면](assets/업그레이드시연/T04b-구간0-1-2-직접실행.png)
> <sub>구간 0 → 1 → 2 가 이어지는 화면. 담당자 직접 실행</sub>
>
...
> <sub>기능별로 "지켜보는 파일 중 공식이 바꾼 것"이 나열됩니다</sub>
>
...
> **공식이 바꾼 834개 중, 우리가 "이건 바뀌면 알려 달라"고 등록해 둔 파일이
> 몇 개나 걸렸는지 세어 주는 것**입니다.

아직 **합치기 전**입니다. 여기서 나오는 숫자는 "닿았다"이지 "깨졌다"가
아닙니다. 실제로 부딪히는지는 구간 5에서 나옵니다.
...
| 이름 | 뜻 |
|---|---|
| `base` | 비교 **출발점**. 지금 우리가 쓰는 공식 버전 |
| `target` | **올라갈** 공식 버전 |
| `changed_path_count` | 공식이 그 사이에 바꾼 파일 수. **우리와 상관없이 공식이 한 일** |
...
| `changed_watch_paths` | 그 기능의 등록표 `upgrade_watch.paths` 에 적힌 파일 중, **공식이 실제로 바꾼 것**의 목록 |
...
**이 표가 이 구간의 쓸모입니다.** 공식이 834개를 바꿨지만, 기준코드 기능
담당자가 실제로 봐야 할 것은 **22개**입니다. 834개를 다 뒤질 필요가
없도록 좁혀 주는 것이 이 검사의 목적입니다.

BANK-OM-005(한글 입력)는 지켜보는 파일이 2개뿐인데 그중 1개가 걸렸습니다.
...
| 항목 | 무엇을 보나 | 어떻게 |
|---|---|---|
| `changed_configuration_keys` | **설정값 이름** | 공식이 바꾼 `.yaml`·`.json`·`.toml` 등의 변경 내용 안에 등록한 설정 이름이 나오는지 |
| `changed_dependencies` | **라이브러리 이름** | 공식이 바꾼 `package.json`·`pom.xml` 등의 변경 내용 안에 등록한 라이브러리 이름이 나오는지 |

**지금 둘 다 비어 있는 이유는 아무도 등록하지 않았기 때문입니다.** 등록표
7장 어디에도 `upgrade_watch.configuration_keys` 와 `upgrade_watch.dependencies`
항목 자체가 없습니다.
...
| 이름 | 뜻 |
|---|---|
| `name` | 어느 검사인지. `upgrade-watch` = "공식이 바꾼 것이 우리 감시 목록에 닿았나" |
...
> 이 검사는 **자동으로 막지 않습니다.** 공식이 그 파일을 바꿨다고 해서
> 우리 기능이 깨진다는 뜻은 아니기 때문입니다. 번역 파일에 다른 언어
