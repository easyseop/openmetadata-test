...
> **지금 이 명령을 그대로 돌리면 끝까지 가지 않습니다.** 실제로 돌려 보면
> 5종 중 하나가 `analysis_error`(종료코드 3)로 나옵니다.
>
> ```json
...
> ```
>
> **OM_TEMP 의 1.13.1 갈래가 아직 원격에 올라가 있지 않기 때문입니다.**
> 검사기가 대조할 상대가 없습니다. 이것이 남아 있는 블로커이고,
> 사람이 브랜치를 올려야 풀립니다. `analysis_error` 는 **통과가 아니라
> "판단 자체를 못 했다"** 는 뜻이므로 통과로 읽으시면 안 됩니다.

### 그 검사기들은 이미 돌려 놓았습니다

**언제 돌렸느냐** — 이 예행연습에서 돌린 것이 아닙니다. **1.13.1 후보를
처음 만들 때 별도로 돌려 결과 파일을 저장소에 넣어 둔 것**입니다.

| 파일 (`harness/registrations/om-temp-1.13.1/`) | 결과 |
|---|---|
| `registration-validation-results.json` | 등록자료 검사 **5/5 통과** |
| `source-gate-results.json` | 소스 게이트 **8/8 통과** |

소스 게이트 8종에 `customization-survival` 이 들어 있습니다. 앞에서 이야기한
**"우리 변경이 새 코드에 살아남았는가"** 검사가 그것이고, 통과했습니다.

**이 결과가 이번 예행연습 결과와 같은 것인지 어떻게 아나** — 결과 파일이
후보의 **파일 내용(tree)에 묶여 있기** 때문입니다.

```json
"candidate": {
  "tree_sha": "e490ed82dd9cfe58833b2050a233aae7496541ab",
  "commit_sha": "dee330ebd5abfe33e1ac61e1ca31879746a1b423"
}
```

**`tree_sha` 가 4부에서 확인하실 값과 똑같습니다.** 기록 번호(`commit_sha`)는
다르지만 파일 내용은 한 글자도 다르지 않으므로, **저 검사 결과는 지금
만드신 후보에 대한 결과**입니다.

4부의 확인값이 왜 중요한지가 여기서 드러납니다. 그 값이 같아야 **"이미
돌려 둔 검사 결과를 내 후보에 그대로 갖다 붙일 수 있다"** 고 말할 수
있습니다.

> **다만 이것으로 배포해도 된다는 뜻은 아닙니다.** 통과한 것은 **코드를
> 읽어서 알 수 있는 것**까지입니다. 빌드·화면·실제 동작 시험과 운영 배포
> 승인은 별도로 남아 있고, OM_TEMP 의 1.13.1 갈래가 아직 원격에 올라가
> 있지 않아 원격 CI 로는 재확인되지 않았습니다.
...
왼쪽은 **한 줄로 곧게 8건**이고 맨 아래가 공식 1.13.1 입니다. 오른쪽은
**두 갈래가 1.13.0 에서 갈라져** 올라가고, 머지할 때마다 이름표 없는
