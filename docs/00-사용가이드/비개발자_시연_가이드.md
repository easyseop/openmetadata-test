...
>
> 현재 결론: **코드를 읽어서 하는 검사는 성공했지만, 실제 서버에 올려서 하는
> 시연은 아직 실행하지 않았다.**

이 문서는 두 가지를 설명한다. 첫째, 명령어를 하나도 입력하지 않고 GitHub
화면만 보며 현재 결과를 확인하는 방법. 둘째, 테스트 서버가 준비되면 운영자와
함께 실제 기능 검사를 실행하는 방법.

용어는 하나로 통일한다 — 은행에서 추가한 기능을 이 문서에서는 **맞춤 변경**
이라고 부르고, 하나하나에 `BANK-OM-001` 같은 번호를 붙인다.
...
업그레이드 방향을 먼저 짚어 둔다. 흔히 “공식 새 버전을 받아서 거기에 우리
기능을 다시 붙인다”고 생각하기 쉽지만, 이 방식은 매번 붙이다가 빠뜨릴 위험이
크다. 그래서 **반대로 한다** — 맞춤 변경이 이미 들어 있는 은행 쪽 브랜치(이
문서에서 `vendor 브랜치`라고 부른다)에 승인된 공식 새 버전을 **가져와
합친다.** 그러면 우리 기능은 원래 자리에 있고, 충돌이 나는 부분만 처리하면
된다.

전체 순서는 다음과 같다. 검사할 코드 한 벌을 “제품 후보(candidate)”라고 부른다.

```text
현재 OpenMetadata + 행내 맞춤 변경
        ↓
업그레이드할 공식 OpenMetadata 버전·SHA 고정
        ↓
공식 새 버전을 행내 vendor 브랜치로 가져와 병합
        ↓
충돌 해결 + 새 버전에 필요한 행내 보강
        ↓
검사할 제품 후보(candidate) 확정
        ↓
① 소스 검사: 맞춤 변경 11개의 등록·누락·commit 규칙 확인
        ↓
② 바로 그 후보를 격리된 테스트 환경에 배포
        ↓
③ 실제 API·연결·화면 검사 9개
        ↓
④ 실제 업그레이드(T90)·같은 산출물 승격(T91)·내부망 검증(T94)
        ↓
최종 결과와 증거
```

**현재 위치는 ①까지다.** 제품 후보를 확정하고 소스 검사를 통과했다.
② 이후는 아직 실행하지 않았다.

### 2.1 맞춤 변경 한 건에 들어가야 하는 것

코드만 고치면 맞춤 변경 등록이 끝나는 것이 아니다. 아래 항목이 한 묶음으로
있어야, 업그레이드한 뒤에 “이 기능이 빠졌는가”를 기계가 검사할 수 있다.

| 들어갈 것 | 쉬운 뜻 | 실제 예시 |
|---|---|---|
| `BANK-OM` ID | 맞춤 변경마다 붙는 고유 번호 | `BANK-OM-011` |
| 제목·상태·종류 | 무엇을 바꿨고 지금도 쓰는지 | 검색 목록 변환 타입 계약 · `active` · `core-patch` |
| owner·승인 경로 | 누가 기능과 결과를 책임지는지 | 현재 `UNASSIGNED`라 배포 전 지정 필요 |
| `changed_paths` | 이 기능이 실제로 건드린 파일 **전부** | `BANK-OM-011`의 UI 파일 6개 |
| `required_changed_paths` | 그중 **하나라도 없어지면 안 되는** 핵심 파일 | 핵심 UI·테스트 파일 6개 |
| `upgrade_watch.paths` | 우리가 바꾼 파일은 아니지만, 공식 쪽에서 바뀌면 우리 기능이 깨질 수 있어 지켜보는 파일 | 목록 hook, 공통 검색 hook |
| 업무 contract | 새 버전에서도 깨지면 안 되는 업무 동작 | InstanceCode·QueryReport 계약 |
| 필수 테스트 | 그 업무 동작을 실제로 확인하는 테스트 파일과 함수 | `test_instance_code.py::test_crud_search_roundtrip` |
| series·의존 ID | 같은 ID로 후속 변경을 더 해도 되는지, 먼저 있어야 할 기능은 무엇인지 | `depends_on: BANK-OM-010` |
| commit 꼬리표 | 제품 코드 commit을 ID와 연결하는 한 줄 | `Customization-ID: BANK-OM-011` |

세 가지 경로 항목이 헷갈리기 쉬우니 한 줄로 구분하면 이렇다.
`changed_paths`는 “우리가 만진 전부”, `required_changed_paths`는 “그중 반드시
살아 있어야 하는 것”, `upgrade_watch.paths`는 “남이 바꾸면 우리가 위험해지는
것”이다.
...
|---|---|---|---|
| 후보 잠금(T24) | “정확히 어느 코드가 검사 대상인가?” | 이후 모든 검사 결과가 이 한 코드에 묶인다 | `849ae756...`로 고정 |
| T25 | “승인한 공식 새 버전이 정말 들어갔나?” | 공식 `1.13.1-release`가 후보 이력 안에 있다 | `pass` |
| T26 | “등록한 맞춤 변경이 병합 후에도 남았나?” | 11개 기능의 필수 파일과 계약이 후보에 있다 | `pass` |
| T30 | “commit마다 `Customization-ID`가 정확히 하나인가?” | ID 없는 commit, ID 두 개인 commit, 제품과 정책을 섞은 commit을 막는다 | `pass` |
| T31 | “같은 ID의 후속 변경과 선후 관계가 정상인가?” | 중간이 빠진 연속 변경, 폐기 ID 재사용, 서로 물고 도는 의존을 막는다 | `pass` |
| T60-I | “필수라고 적어 둔 테스트가 실제로 존재하나?” | 없는 테스트를 “검사했다”고 착각하지 않는다 | 9개 모두 `pass` |
| T61 patch-kill | “기능을 빼면 테스트가 정말 실패하나?” | 그 테스트가 껍데기가 아니라 진짜 기능을 잡는다 | Sybase·Tibero 2건만 확인, 서버가 필요한 3건은 남음 |
| T62 Runtime contracts | “실제 배포본에서 API·화면이 맞는가? 그 결과가 같은 후보에 묶였나?” | API 4·연결 2·브라우저 3, 총 9개의 실제 동작 | 아직 한 번도 실행 안 함 |
| T63 UI 기준선 | “은행 코드가 공식 원본보다 UI 타입 오류를 새로 늘렸나?” | 공식 원본과 파일·오류 종류를 하나씩 맞춰 비교한 결과 | 새 오류 0건, 사람 승인 필요 |
| T90 | “실제 데이터로 업그레이드 12단계가 성공했나?” | DB 변경·검색 재색인·수집·권한까지 포함한 근거 | 미실행 |
| T91 | “검사한 바로 그 파일을 배포했나?” | 이미지·패키지·Helm이 중간에 바뀌지 않았다 | 미실행 |
| T94 | “내부망에 들여온 파일이 서명한 그 파일인가?” | 오프라인 반입 중 변조·누락이 없다 | 미실행 |

정리하면 현재 상태는 한 문장으로 이렇다.

> **공식 1.13.1과 맞춤 변경 11개가 제품 후보에 함께 들어갔다는 것까지는
> 확인했다. 그러나 실제 서버에서의 API·화면 검사, 그리고 업그레이드·승격·
> 내부망 검증은 아직 하지 않았다.**
...
이 화면은 특히 중요하다. 앞의 3단계에서 본 Actions 화면은 **초록색**이었는데,
여기 실제 환경 검사 부분은 `verdict: block`이다. 모순이 아니라 서로 다른
것을 본 결과다.

- 3단계의 초록색 = 코드를 읽어서 하는 검사가 모두 성공했다
- 여기의 `verdict: block` = 실제 서버에서 하는 검사 9개 중 2개만 돌았고 7개는
  아예 못 돌았으므로, 이 부분은 통과라고 말할 수 없다

화면에서 `passed: 2`, `skipped: 7`, `verdict: block`,
`operational_run_executed: false` 네 줄을 함께 보여주면 된다. 시스템이 실패를
숨기지 않고 “아직 못 했다”를 그대로 기록한다는 근거이기도 하다.
...
한 번 실행하면 아래 9개 검사 항목이 모두 돌아간다. 골라서 일부만 돌릴 수는
없다.
...
2. 3단계 표의 9개 검사 항목이 모두 실행됐는지 확인한다.
