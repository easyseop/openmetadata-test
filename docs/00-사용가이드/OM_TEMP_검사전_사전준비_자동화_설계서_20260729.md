...
> 문서 상태: Claude 독립 검토 반영 · 0단계 기준선 복구 완료
...
이 문서의 `plan`·`apply` 명령은 아직 구현되지 않았습니다. 2026-07-29
독립 검토에서 발견한 기준선 결함과 OM_TEMP 1.13.0 증적은 복구했습니다.
이제 각 후속 단계의 완료 조건을 통과한 뒤에만 쓰기 기능을 구현합니다.
...
현재 `generate_manifest_drafts.py`는 BANK-OM별 Git commit SHA를 Python 코드에 직접 적어 둡니다. 따라서 기존 BANK-OM ID에 후속 commit이 생겨도 생성기가 자동으로 발견하지 못합니다.
...
`generate_manifest_drafts.py`의 하드코딩된 `CUSTOMIZATIONS[*].commits`는 제거합니다. 사람 정책값은 Python 상수가 아니라 기존 Manifest와 Registry에서 읽습니다.
...
- symlink, submodule, Git LFS 경로의 mode 기반 차단은 현재 미구현입니다.
  쓰기 기능보다 먼저 차단 코드와 실패 test를 추가합니다.
