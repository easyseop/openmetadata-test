...
| `upgrade_watch.paths` | 다음 공식 버전이 바뀔 때 이 기능과의 연결을 다시 검토해야 할 파일은 무엇인가? | 현재 검사기에서는 수정한 공식 파일과 미수정 의존 파일을 사람이 등록 | 공식 버전 사이에서 해당 경로가 바뀌면 `APPROVAL` |
...
현재 T42 검사는 `upgrade_watch.paths`에 사람이 적은 경로만 공식 버전 전후로
비교합니다. `allowed`에 적었다고 자동으로 T42 비교 대상이 되지는 않습니다.

따라서 OM_TEMP 1.13.0 Manifest를 처음 등록할 때는 다음 기준을 사용합니다.

1. `git show --name-status`에서 `M`으로 나온 파일은 공식 1.13.0에도 존재했던
   파일을 수정한 것이므로 `upgrade_watch.paths`에도 등록합니다.
2. `A`로 나온 커스터마이징 신규 파일도 공식 1.13.1이 같은 경로를 새로 만들면
   이름 충돌이 생깁니다. 이번 업그레이드 시연에서는 안전하게 watch에
   등록합니다.
3. 커스터마이징이 직접 수정하지 않았지만 호출하거나 구조에 의존하는 공식 파일이
   있으면 추가로 등록합니다.
4. 공식 1.13.0과 1.13.1 사이에서 해당 경로가 바뀌면 T42가 자동 통과하지 않고
   담당자 검토를 요구합니다.

결과적으로 현재 검사기에서는 `allowed` 전체를 watch에도 한 번 등록한 뒤,
미수정 의존 파일을 추가하는 방식이 가장 누락 위험이 적습니다. 두 목록을
수동으로 중복 작성하지 않도록 `allowed`의 공식 파일을 T42에 자동 포함하는
개선은 아직 구현 전입니다.

검사기에는 공식 버전에서 바뀐 파일과 커스터마이징 코드의 직접 참조를 분석해
추가 watch 경로를 제안하는 기능이 있습니다. 이 기능은 Manifest를 자동
수정하지 않으며, 1.13.1 업그레이드 비교 단계에서 담당자가 제안을 검토해
수동으로 반영합니다.
...
- BANK-OM-001~007 Manifest 초안이 1.13.0 실제 commit을 기준으로 작성됨
