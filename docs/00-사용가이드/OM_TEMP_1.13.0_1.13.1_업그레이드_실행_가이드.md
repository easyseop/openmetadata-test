...
업그레이드 때 확인할 경로와 연결 test를 기록한 맞춤 변경 등록 문서입니다.
...
> **834와 111을 혼동하지 마십시오.** 서로 다른 두 비교의 결과입니다.
> **834개**는 공식 1.13.0과 공식 1.13.1 사이에서 OpenMetadata 쪽이 바꾼 경로
> 수입니다. **111개**는 같은 버전 안에서 공식 코드와 행내 코드 사이의 차이,
> 즉 우리가 바꾼 경로 수입니다. 앞은 “남이 얼마나 바뀌었나”, 뒤는 “우리가
> 얼마나 바꿨나”이므로 둘을 더하거나 비교하지 않습니다.

`APPROVAL`은 실패가 아닙니다. 자동으로 진행하기 전에 담당자가 영향 경로를
직접 확인하라는 뜻입니다.

`upgrade_watch.paths`는 현재 다음 방식으로 등록합니다.

- 준비도구의 `plan`이 해당 BANK-OM commit의 실제 변경 경로 가운데 **공식 patch
  branch에 존재하는 경로만** 자동으로 포함합니다. 공식 코드에 없는 행내 신규
  파일은 watch에 자동으로 넣지 않고 BANK-OM별 담당자 판단 질문으로 남깁니다.
  공식 tree에 없는 경로를 watch에 넣으면 T93 경로 최신성 검사가 매번
  `stale_pattern` APPROVAL을 내기 때문입니다.
- 행내에서 수정하지 않았지만 기능이 의존하는 경로는 담당자가
  `watch_dependencies`에 적습니다.
- 새 공식 버전에서 바뀐 파일 이름을 행내 코드가 직접 참조하면 검사기가
  추가 watch 후보와 참조 근거를 제시합니다. 담당자가 확인한 뒤 Manifest에
  반영합니다.

따라서 공식 tree에 있는 변경 경로의 자동 포함과 직접 참조 후보 제시는 현재 구현되어 있습니다.
...
`patch/om-1.13.1`을 비교해도 같은 결과가 나옵니다. 맞춤 변경이 들어간
...
가이드에서 확인할 수 있도록 별도로 보관한 증거 자료**입니다. 모두
`harness/registrations/om-temp-1.13.1/conflict-evidence/` 아래에 있습니다.

| 파일 | 내용 |
|---|---|
| `BANK-OM-001_ko-kr_full_conflict.txt` | Git이 남긴 충돌 표시 원문 6,614행 전체 |
| `BANK-OM-001_ko-kr_resolution.diff` | 해결 결과의 diff |
| `BANK-OM-001_ko-kr_resolved.json` | 해결한 뒤의 전체 JSON |
| `BANK-OM-001_ko-kr_capture.json` | commit과 18개 충돌 경로의 연결 기록 |
...
실제 파일은 `harness/registrations/om-temp-1.13.1/`에 있습니다.

> **이 결과는 지금 다시 실행할 수 없습니다.** `patch/om-1.13.1`과
> `custom/om-1.13.1`은 로컬에만 만들었고 원격에 push하지 않았습니다. 그 노트북
> 밖에서는 `dee330ebd5...`를 가져올 방법이 없으므로, 이 절의 숫자를 다시
> 확인하려면 먼저 두 branch를 원격에 올려야 합니다. 1.13.0과 달리 1.13.1은
> 재현 절차 문서도 없습니다.
...
| 맞춤 변경 생존 | 핵심 파일과 Contract 연결이 남았는지 | PASS |
...
| 운영 통합 방식 일치 | 실제 vendor-merge 기록과 충돌 해결 증거가 있는지 | 미검증 |

마지막 줄의 **미검증**은 검사기가 내는 판정값(`pass`/`approval`/`block`/
`analysis_error`) 중 하나가 아닙니다. 이 후보를 vendor-merge로 만들지 않았기
때문에 애초에 그 검사를 돌리지 않았다는 뜻입니다. 3절에서 설명한 것처럼
저장된 결과의 `integration_strategy`에는 `vendor-merge`라고 적혀 있지만 실제
제작 방법은 commit별 재적용이었습니다. **이 줄을 통과로 읽으면 안 됩니다.**
...
`SKIP`은 성공이 아닙니다. 7개는 실제 OpenMetadata 서버와 브라우저 화면이
있어야 돌아가는 test입니다. `OPENMETADATA_BASE_URL`과 세 가지 브라우저 검사용
URL이 준비된 행내 환경에서 다시 실행해야 합니다.

전체 build도 아직 실행 전입니다. 검사를 수행한 환경에 Java Runtime, Maven,
Yarn과 UI의 `node_modules`가 없어서 build 명령을 시작조차 하지 못했습니다.
**“build가 실패했다”가 아니라 “build를 시도하지 못했다”**입니다. 이 둘은
전혀 다른 결과이므로 구분해서 보고해야 합니다.

## 8. 현재 완료와 다음 단계

우리의 목적은 “공식 업그레이드 후 행내 맞춤 변경이 코드에 빠짐없이
적용됐고, 업무 동작까지 정상인지 확인한 뒤 배포 검토로 넘기는 것”입니다.

| 목적에 필요한 확인 | 현재 결과 | 지금 말할 수 있는 범위 |
|---|---|---|
| 공식 변경이 맞춤 변경에 영향을 주는지 | 충족 | upgrade-watch가 7개 BANK-OM을 검토 대상으로 표시 |
...
| 이 결과를 다른 사람이 재현할 수 있는지 | 미충족 | 1.13.1 두 branch가 원격에 없음 |
...
다음 단계는 순서대로 다음과 같습니다.

1. **1.13.1 두 branch를 원격에 push합니다.** 이것을 먼저 해야 나머지를 다른
   사람이 확인할 수 있습니다.
2. Java·Maven·Yarn과 행내 test URL을 준비해 build와 남은 Contract test 7개를
   실행합니다.
3. 이번과 별개로, 직전 custom branch에 공식 1.13.1을 **실제로 merge해**
   vendor-merge 후보를 만듭니다. 그리고 merge 기록·충돌 증거·candidate lock의
   전략값이 서로 맞는지 검사합니다. 6절의 “미검증” 줄이 이때 채워집니다.
4. 충돌 비교 plan과 승인 기록 기능(4절 끝의 6단계)을 구현합니다.
5. 실제 Git 화면·터미널·검사 결과 캡처를 시연 문서에 추가합니다.
