...
> **2026-08-08 고객용 버전 업그레이드 가이드:** 기존 1.13.1→1.13.2
> 예행연습 11단계는 보존하고, 새 Phase 통합 실행 방식을 고객사
> 관점의 네 단계로 설명하는 독립 Markdown·HTML을 추가했다. 새 가이드는
> 제공팀·고객사·자동화의 역할, 각 단계의 완료·중단 기준, 상태별
> 재시작 절차와 증거 목록을 포함한다. 사람이 반드시 결정할 ID·required·
> watch·공용 코드·Contract·충돌·승인·배포 항목과 미입력 시 중단 기준을
> 본문 요약과 상세표로 보강했다. 실제 1.13.2 진행 상태는
> 기준선·병합 전 검사 완료, vendor-merge 후보 미생성으로 분리했다.
